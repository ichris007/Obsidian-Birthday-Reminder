...
插件现已上架 **Obsidian 社区插件市场**！🎉

#### 方法一：从社区插件市场安装（推荐）
1. 打开 **设置** → **社区插件**
2. 点击 **浏览**，搜索 `Birthday Reminder`
3. 点击 **安装**，然后点击 **启用**

#### 方法二：通过 BRAT 安装
1. 打开 **设置** → **BRAT**（在第三方插件列表中找到）
2. 点击 **Add Beta Plugin** 按钮
3. 在弹出的对话框中输入仓库地址：`https://github.com/ichris007/Obsidian-Birthday-Reminder`
4. 点击 **Add Plugin** – BRAT 会自动下载并安装
5. 进入 **设置** → **社区插件**，找到 **Birthday Reminder** 并启用
...
### v1.5.2 (2026-07-16)
- 🐛 **修复年龄计算逻辑** – 正确处理今年生日已过的情况，避免年龄多算 1 岁
- 🔧 代码清晰度提升，显式提取 `birthMonth`/`birthDay` 变量

### v1.5.1 (2026-07-16)
- 🔧 **安装问题修复** – 将 `minAppVersion` 降低至 `1.12.7`，解决 "no appropriate version found" 安装错误
- 🌐 **完整本地化** – 所有设置界面文本完整本地化；配色方案名称现在根据所选语言显示
- ✨ **滑块动态提示** – 拖动"可见月数"滑块时实时显示当前值（如"3 个月"）
- 🎨 **配色预览实时更新** – 切换配色方案时预览立即更新
- 🔄 **语言切换即时生效** – 切换语言后设置界面立即刷新

<details>
    <summary>点击查看更多</summary>
    
### v1.5.0 (2026-07-15)
- 🏗️ **设置 API 稳定化** – 回归传统 `Setting` 类 + `display()` 方法，确保稳定性
- ⚙️ **设置项完整实现** – 所有设置项功能完整，带有正确的类型注解
- 🎨 **配色预览** – 可视化预览所选配色方案
- 🔮 **预留升级路径** – 保留 `getSettingDefinitions()` 供未来迁移

### v1.4.1 (2026-07-15)
- 🛡️ **零 `any` 类型** – 完整的类型安全重构
- 🏗️ **设置 API 迁移** – 迁移至新式声明式 API
- 🪟 **弹出窗口兼容** – 完整支持 Obsidian 弹出窗口

### v1.4.0 (2026-07-15)
- 🔄 **自动刷新** – 生日笔记被创建、删除、重命名或修改时视图自动更新
- 📦 **更新 minAppVersion** 至 `1.13.0`

### v1.3.0 (2026-07-15)
- ✨ **文件变更自动刷新** – 添加文件监听器，100ms 防抖
- 🔧 **Promise 处理规范化** – 所有异步操作正确标记
- 🛡️ **类型安全** – 添加类型断言和守卫

### v1.2.0 (2026-07-15)
- 🎯 **代码质量** – 所有社区插件审查问题已解决
- 🎨 **UI 重构** – 内联样式迁移为 CSS 类
- 🧹 **内存泄漏修复** – 移除多余引用，优化清理逻辑
...
### v1.0.0 (2026-03-21)
- ✨ **首次发布**
- 🎯 可配置生日属性（`birthday`、`date_of_birth`、`DOB` 等）
- 🎯 智能生日计算和年龄计算
